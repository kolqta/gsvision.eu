...
class ProductCore extends ObjectModel
{
    ...
    */
    ...
    /**
	 * product_features, features - няма такива таблици
	 * var/cache/prod/class_index.php - delete
	 *
	 * List products features AND List features_value
	 *
     * @param int $id_product
     *
     * @return array
     */
    public static function getProductFeaturesById($id_product = 0)
    {
        // List products features
		$id_lang = (int) Context::getContext()->language->id;
        return Db::getInstance()->executeS('
			SELECT fp.*, f.*, fl.*, fv.*, fvl.*
			FROM `' . _DB_PREFIX_ . 'feature_product` fp
				LEFT JOIN `' . _DB_PREFIX_ . 'feature` f
					ON (fp.`id_feature` = f.`id_feature`)
					LEFT JOIN `' . _DB_PREFIX_ . 'feature_lang` fl
						ON (f.`id_feature` = fl.`id_feature` AND fl.`id_lang` = ' . $id_lang . ')
				LEFT JOIN `' . _DB_PREFIX_ . 'feature_value` fv
					ON (fp.`id_feature_value` = fv.`id_feature_value`)			
					LEFT JOIN `' . _DB_PREFIX_ . 'feature_value_lang` fvl
						ON (fv.`id_feature_value` = fvl.`id_feature_value` AND fvl.`id_lang` = ' . $id_lang . ')
